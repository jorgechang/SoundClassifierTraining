...
class AirAbsorption(BaseWaveformTransform):
    """Applies an air absorption parametrized by temperature, humidity, and distance.
    ...
    where distance is the microphone-source assumed distance in meters
    and `absorption_coefficient` is adapted from a lookup table by pyroomacoustics [1].
    ...
    attenuate according to the distance law. Gain augmentation done separately.
    ...
    #
    # And the values are attenuation coefficients `coef` that attenuate
    # the corresponding band in "center_freq" by
    # exp(-coef * <microphone-source distance>).
    # The original table does not have the last two columns
    # which have been extrapolated from the
    ...
    def __init__(
        ...
        :param min_temperature: Minimum temperature in celcius (values ranges 10 to 20)
        :param max_temperature: Maximum temperature in celsius (values ranges 10 to 20)
